    ...
    getCommentListFromServer(bno, page).then(result=>{
        ...
        if(result.cmtList.length > 0){
            ...
            for(let cvo of result.cmtList){
                ...
                li += `<button type="button" data-cno="${cvo.cno}" class="btn btn-outline-info mod" data-bs-toggle="modal" data-bs-target="#myModal">e</button>`;
                li += `<button type="button" data-cno="${cvo.cno}" class="btn btn-outline-danger del">x</button>`;
                li += `</li>`;
                ul.innerHTML += li;
            }
            //더보기 버튼 코드
